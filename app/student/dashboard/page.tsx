...
import { CalendarDays, Send, ShieldAlert, User, LayoutDashboard } from "lucide-react";
        ...
        <div className="min-h-screen bg-[#f8fafc] text-slate-900">
            <nav className="bg-slate-900 text-white px-8 py-4 flex justify-between items-center">
                <div className="font-black text-xl uppercase tracking-tighter flex items-center">
                    <LayoutDashboard className="w-6 h-6 mr-2 text-blue-400" /> Student Portal
                </div>
                <LogoutButton />
            </nav>
            ...
            <main className="max-w-5xl mx-auto px-6 py-12">
                <div className="mb-12">
                    <h1 className="text-5xl font-black italic uppercase tracking-tighter">My Portal</h1>
                    <p className="text-slate-500 font-bold mt-2 uppercase tracking-widest text-xs">
                        Registered to: {user?.name} ({user?.username})
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <ActionCard
                        ...
                        desc="Check your official presence logs."
                    />
                    <ActionCard
                        title="Request Leave"
                        icon={<Send className="w-8 h-8" />}
                        href="/student/leave"
                        desc="Submit new leave applications."
                    />
                </div>
            </main>
        </div>
